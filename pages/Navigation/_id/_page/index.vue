...
export default {
	...
	async asyncData({
		...
		var backData = await axios.get(`http://backup.17getfun.com/discovery/channelContentsPaging?channelId=${params.id}&page=${params.page}&pageSize=30&tagSize=35`);        
		return {
			data: data.data,
...
//		let activepage=(window.location.href).substr(36,3)
//		console.log(document.getElementsByClassName("activepage"))
//		http://localhost:3000/Navigation/31/2
		if(document.getElementsByClassName("swiper")[0].innerHTML == "") {
		document.getElementsByClassName("swiper")[0].style.height=0;
			document.getElementById("div2")[0].style.height = 0;
		} else {
